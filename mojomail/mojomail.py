...
class MojoMailer:
  ...
  def getmessagelist(self,folder,search,since,before):
    ...
    return messagelist
  def getmessageshorts(self,items):
...
class MojoMessager:  
  def __init__(self,configfile):
    config=ConfigParser.ConfigParser()
    config.read(configfile)
    self.subskelpath=config.get("System","subskelpath")
    self.bodyskelpath=config.get("System","bodyskelpath")
